...
case class KafkaRequestBuilder(requestName: Expression[String]) {
  ...
  def sendAvro[V](payload: Expression[String])(implicit format: RecordFormat[V], decoder: Decoder[V]): KafkaRequestAvroActionBuilder[_, V] = sendAvro(payload, None)

  def sendAvro[K, V](key: Expression[K], payload: Expression[String])(implicit format: RecordFormat[V], decoder: Decoder[V]): KafkaRequestAvroActionBuilder[K, V] = sendAvro[K, V](payload, Some(key))
  ...
  private def sendAvro[K, V](payload: Expression[String], key: Option[Expression[K]])(implicit format: RecordFormat[V], decoder: Decoder[V]) =
    new KafkaRequestAvroActionBuilder[K, V](KafkaAvroAttributes[K, V](requestName, key, payload, decoder, format))
